...
function onYourPlayer(data) {
	...
	p1Unfog.x -= 75; // Relative to parent
	p1Unfog.y -= 75; // Relative to parent
...
function onNewPlayer(data) {
	...
	p2Unfog.x -= 75; // Relative to parent
	p2Unfog.y -= 75; // Relative to parent
...
if (player1Bool && player2Bool) {
	...
	game.physics.arcade.overlap(player1,shield_item,function(player1,shield_item){
		...
		p1Shield_on.x -= 11; // Relative to parent
		p1Shield_on.y -= 11; // Relative to parent
	...
	game.physics.arcade.overlap(player2,shield_item,function(player2,shield_item){
		...
		shield_on_sound.play();
		p2Shield_on.x -= 11; // Relative to parent
		p2Shield_on.y -= 11; // Relative to parent
...
function collisionHandler (player1,bullet) {
	bullet.kill();
	player2_life--;
	if (player2_shield_on == 0){
		socket.emit('player health',{pID: player2ID, pHealth: player2_life});
...
function collisionFogHandler(player, fogBlock) {
	//TODO: Calculate a fix diameter from center of opbect to remove blocks
    if (fogBlock.frame == 0) {
